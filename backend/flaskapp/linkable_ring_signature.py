...
def export_signature(y, message, signature, foler_name='./data', file_name='signature.txt'):
    ...
    data = '\n'
    data += "You will be voting for candidate {}\n".format(message)
    data += '\n'
...
def check_keyImage(signature,keyimage):
    n_keyimage = [0, 0]
...
def main():
    ...
    export_private_keys(x , './data', 'secret.txt')
    export_private_keys(y, './data', 'public.txt')
    ...
    signature2 = ring_signature(x[i], i, message, y)
    keyimage = [0,0]
    keyimage[0] = hex(signature2[2].x())
    keyimage[1] = hex(signature2[2].y())
    check_keyImage(signature,keyimage)

    assert(verify_ring_signature(message, y, *signature2))
    export_signature(y, message, signature, './data', 'signature.txt')
    export_signature(y, message, signature2, './data', 'signature2.txt')
